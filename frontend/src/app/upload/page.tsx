...
import { UploadCloud, Loader2, Cpu, ShieldCheck, AlertTriangle, MapPin, User, Calendar, BookmarkPlus, CheckCircle2, ChevronDown } from 'lucide-react';
...
const RECORD_TYPES = [
  { value: "OLD_SCAN_712", label: "OLD SCANNED VF-7/12 DETAILS (àªà«àª¨àª¾ àª¸à«àªà«àª¨ àªàª°à«àª² àªàª¾.àª¨. à«­/à«§à«¨ àª¨à« àªµàª¿àªàª¤)" },
  { value: "VF7",          label: "VF-7 SURVEY NO DETAILS (àªàª¾.àª¨. à«­ àª¨à« àªµàª¿àªàª¤)" },
  { value: "VF8A",         label: "VF-8A KHATA DETAILS (àªàª¾.àª¨. à«®àª àª¨à« àªµàª¿àªàª¤)" },
  { value: "VF6",          label: "VF-6 ENTRY DETAILS (àª¹àªà«àª àªªàª¤à«àª°àª àªàª¾.àª¨. à«¬ àª¨à« àªµàª¿àªàª¤)" },
  { value: "135D",         label: "135-D NOTICE FOR MUTATION (àª«à«àª°àª«àª¾àª° àª®àª¾àªà« à«§à«©à««-àª¡ àª¨à«àªàª¿àª¸)" },
  { value: "INTEGRATED",   label: "INTEGRATED SURVEY NO DETAILS (àª¸àª°à«àªµ àª¨àªàª¬àª°àª¨à« àª²àªà«àª¤à« àª¸àªàªªà«àª°à«àª£ àª®àª¾àª¹àª¿àª¤à«)" },
  { value: "OLD_SCAN_6",   label: "OLD SCANNED VF-6 ENTRY DETAILS (àªà«àª¨àª¾ àª¸à«àªà«àª¨ àªàª°à«àª² àª¹àªà«àª àªªàª¤à«àª°àª àªàª¾.àª¨. à«¬ àª¨à« àªµàª¿àªàª¤)" },
  ...
  { value: "REVENUE_CASE", label: "REVENUE CASE DETAILS (àªàª®à«àª¨ àª°à«àªà«àª°à«àª¡ àª¨à« àª²àªà«àª¤àª¾ àªà«àª¸àª¨à« àªµàª¿àªàª¤)" },
];
...
export default function DocumentUpload() {
  ...
  // ââ Cascade dropdown data ââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  const [villageManual, setVillageManual]   = useState(false); // fallback manual entry
  ...
  // ââ Fetch districts on mount ââââââââââââââââââââââââââââââââââââââââââ
  useEffect(() => {
  ...
  // ââ Fetch talukas when district changes âââââââââââââââââââââââââââââââ
  useEffect(() => {
    if (!district) {
      setTalukas([]);
      setTaluka('');
    ...
    setVillageError('');
    fetch(`${API_BASE_URL}/talukas/${encodeURIComponent(district)}`)
      .then(r => r.json())
  ...
  // ââ Fetch villages when taluka (or recordType) changes ââââââââââââââââ
  useEffect(() => {
    if (!district || !taluka) {
      setVillages([]);
      setVillage('');
    ...
    setVillageError('');
    setVillageManual(false);
    const url = `${API_BASE_URL}/villages?district=${encodeURIComponent(district)}&taluka=${encodeURIComponent(taluka)}&record_type=${recordType}`;
    fetch(url)
      .then(r => {
      ...
      .then(d => {
        ...
        if (!d.villages?.length) setVillageManual(true);
      })
      .catch(() => {
        setVillages([]);
        setVillageError('Could not load villages from AnyROR portal â enter village name manually.');
        setVillageManual(true);
      })
      .finally(() => setLoadingVillages(false));
  }, [district, taluka, recordType]);
  ...
  const getEntryFieldLabel = () => {
    switch (recordType) {
      ...
      case 'OLD_SCAN_712':return 'Survey / Block No. (àª¸à«àªà«àª¨ àª°à«àªà«àª°à«àª¡ àª®à«àªàª¬ àª¸àª°à«àªµà«/àª¬à«àª²à«àª àª¨àª)';
      case 'OLD_SCAN_6':  return 'Survey / Block No. (àª¸à«àªà«àª¨ àª°à«àªà«àª°à«àª¡ àª®à«àªàª¬ àª¸àª°à«àªµà«/àª¬à«àª²à«àª àª¨àª)';
  ...
  const isAutoFormValid = !!(
    ...
    (needsOwnerName ? ownerName.trim() : surveyNo.trim())
  );
  ...
  const handleSaveToPortfolio = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('portfolio_assets').insert({
        ...
        district:        autoResult.district  || district,
        taluka:          autoResult.taluka    || taluka,
        village:         autoResult.village   || village,
  ...
  const inputClass  = "w-full bg-[#111] border border-[#3b494b] text-[#dbfcff] px-3 py-2.5 font-mono text-sm focus:outline-none focus:border-[#00f0ff] transition-colors placeholder:text-[#3b494b] disabled:opacity-40";
  const selectClass = "w-full bg-[#111] border border-[#3b494b] text-[#dbfcff] px-3 py-2.5 font-mono text-sm focus:outline-none focus:border-[#00f0ff] transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed";
      ...
      <div className="z-10 w-full max-w-[640px] flex flex-col gap-6 mt-4">
        ...
        {activeTab === 'manual' ? (
          ...
          <form onSubmit={handleAutomate} className="glass-panel p-8 border border-[#3b494b]/40 flex flex-col gap-5">
            ...
            {/* ââ Record Type âââââââââââââââââââââââââââââââââââââââââââ */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase tracking-widest font-bold text-[#00f0ff]">
                Select Record Type (àªà«àª àªàª àªªàª¸àªàª¦ àªàª°à«)
              </label>
              <select
                value={recordType}
                onChange={e => { setRecordType(e.target.value); setVillage(''); setVillages([]); }}
            ...
            {/* ââ District âââââââââââââââââââââââââââââââââââââââââââââââ */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase tracking-widest font-bold text-[#849495]">
                District (àªà«àª²à«àª²à«)
              </label>
              {districts.length > 0 ? (
                <select value={district} onChange={e => setDistrict(e.target.value)} className={selectClass}
                  style={{ appearance: 'auto', WebkitAppearance: 'menulist' as any }}>
                  <option value="">â Select District â</option>
            ...
            {/* ââ Taluka âââââââââââââââââââââââââââââââââââââââââââââââââ */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase tracking-widest font-bold text-[#849495] flex items-center gap-2">
                Taluka (àª¤àª¾àª²à«àªà«)
                {loadingTalukas && <Loader2 size={10} className="animate-spin text-[#00f0ff]"/>}
              </label>
              {talukas.length > 0 ? (
                <select value={taluka} onChange={e => setTaluka(e.target.value)}
                  disabled={!district || loadingTalukas}
                  className={selectClass}
                  style={{ appearance: 'auto', WebkitAppearance: 'menulist' as any }}>
                  <option value="">â Select Taluka â</option>
                  {talukas.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              ) : (
                <input type="text" value={taluka} onChange={e => setTaluka(e.target.value)}
                  placeholder={district ? "e.g. Daskroi" : "Select district first"}
                  disabled={loadingTalukas}
                  className={inputClass} />
              )}
            </div>

            {/* ââ Village ââââââââââââââââââââââââââââââââââââââââââââââââ */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase tracking-widest font-bold text-[#849495] flex items-center gap-2">
                Village (àªàª¾àª®)
                {loadingVillages && (
                  <span className="flex items-center gap-1 text-[#eab308]">
                    ...
                    <span>Fetching from AnyROR portal...</span>
                  </span>
                )}
              </label>

              {!villageManual && villages.length > 0 ? (
                <select value={village} onChange={e => setVillage(e.target.value)}
                  className={selectClass}
                  style={{ appearance: 'auto', WebkitAppearance: 'menulist' as any }}>
                  <option value="">â Select Village â</option>
                  {villages.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              ) : (
                <input type="text" value={village} onChange={e => setVillage(e.target.value)}
                  placeholder={
                    loadingVillages ? "Loading villages from AnyROR..." :
                    (!taluka ? "Select taluka first" : "Type village name (Gujarati or English)")
                  }
              ...
              {villageError && (
                <span className="text-[10px] text-[#eab308] font-mono">{villageError}</span>
              )}

              {/* Toggle between dropdown and manual entry */}
              {villages.length > 0 && (
                <button type="button" onClick={() => setVillageManual(v => !v)}
                  className="text-[10px] text-[#849495] hover:text-[#00f0ff] text-left underline transition-colors w-fit">
                  {villageManual ? "â Use dropdown list" : "Type village name manually â"}
                </button>
              )}
            </div>

            {/* ââ Survey / Owner ââââââââââââââââââââââââââââââââââââââââââ */}
            <div className="flex flex-col gap-1.5">
              {needsOwnerName ? (
                <>
                  <label className="text-[10px] uppercase tracking-widest font-bold text-[#849495]">
                    Owner Name (àªàª®à«àª¨ àª®àª¾àª²àª¿àªàª¨à«àª àª¨àª¾àª®)
                  </label>
                  <input type="text" value={ownerName} onChange={e => setOwnerName(e.target.value)}
                    placeholder="Enter owner name" className={inputClass} />
                </>
              ) : (
                <>
                  <label className="text-[10px] uppercase tracking-widest font-bold text-[#849495]">
                  ...
                  <input type="text" value={surveyNo} onChange={e => setSurveyNo(e.target.value)}
                    placeholder="e.g. 123 or 123/A" className={inputClass} />
                </>
              )}
            </div>
            ...
            <div className="text-[10px] text-[#849495] bg-[#1c1b1b]/60 p-3 border border-[#3b494b]/30">
              <span className="text-[#eab308] font-bold">â¡ CAPTCHA:</span> The RPA bot will auto-solve the government CAPTCHA using Gemini Vision AI. This may take 10â20 seconds.
              <span className="text-[#00f0ff]"> All processing happens in background â you&apos;ll see a live progress feed.</span>
            </div>

            <button type="submit" disabled={!isAutoFormValid}
              className="mt-2 w-full py-4 text-[#002022] font-bold text-sm tracking-[0.15em] uppercase bg-gradient-to-r from-[#de4ced] to-[#ff00f0] hover:brightness-110 transition-all flex justify-center items-center disabled:opacity-50 disabled:cursor-not-allowed">
